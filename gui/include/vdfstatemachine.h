...
namespace VDFStateMachine {
    ...
    namespace APPID {
        inline void handleState(uint8_t& value, ParseState& state, FieldType& type) {
            if (value == 0x01) {
    ...
    namespace WAITING {
        inline void handleState(uint8_t& value, ParseState& state, FieldType& type, std::vector<std::string>& listValue) {
            if (value != 0x08) {
                //Update the state
                state = ParseState::KEY;
                if (value == 0x01) type = FieldType::STRING;
    ...
    namespace KEY {
        inline void handleState(uint8_t& value, ParseState& state, FieldType& type, std::ostringstream& utf8String, std::string& key) {
            if (value != 0x00) {
    ...
    namespace VALUE {
        ...
        inline void handleState(uint8_t& value, ParseState& state, FieldType& type, std::ostringstream& utf8String,
    ...
    namespace ENDING {
        inline void handleState(uint8_t& value, ParseState& state, FieldType& type, std::vector<char>& endingBuffer) {
