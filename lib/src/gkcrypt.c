...
static ChiakiErrorCode gkcrypt_gen_key_iv(ChiakiGKCrypt *gkcrypt, uint8_t index,
                                          const uint8_t *handshake_key,
                                          const uint8_t *ecdh_secret);

static void *gkcrypt_thread_func(void *user);

CHIAKI_EXPORT ChiakiErrorCode chiaki_gkcrypt_init(
    ChiakiGKCrypt *gkcrypt, ChiakiLog *log, size_t key_buf_chunks,
    uint8_t index, const uint8_t *handshake_key, const uint8_t *ecdh_secret) {
  gkcrypt->log = log;
  gkcrypt->index = index;
  ...
  if (gkcrypt->key_buf_size) {
    gkcrypt->key_buf =
        chiaki_aligned_alloc(KEY_BUF_CHUNK_SIZE, gkcrypt->key_buf_size);
    if (!gkcrypt->key_buf) {
  ...
  chiaki_gkcrypt_gen_gmac_key(0, gkcrypt->key_base, gkcrypt->iv,
                              gkcrypt->key_gmac_base);
  gkcrypt->key_gmac_index_current = 0;
  memcpy(gkcrypt->key_gmac_current, gkcrypt->key_gmac_base,
         sizeof(gkcrypt->key_gmac_current));

  if (gkcrypt->key_buf) {
    err = chiaki_thread_create(&gkcrypt->key_buf_thread, gkcrypt_thread_func,
                               gkcrypt);
    if (err != CHIAKI_ERR_SUCCESS)
      goto error_key_buf_cond;
...
static ChiakiErrorCode gkcrypt_gen_key_iv(ChiakiGKCrypt *gkcrypt, uint8_t index,
...
#else
  if (!HMAC(EVP_sha256(), ecdh_secret, CHIAKI_ECDH_SECRET_SIZE, data,
            sizeof(data), hmac, (unsigned int *)&hmac_size))
    return CHIAKI_ERR_UNKNOWN;
  ...
  memcpy(gkcrypt->iv, hmac + CHIAKI_GKCRYPT_BLOCK_SIZE,
...
CHIAKI_EXPORT void chiaki_gkcrypt_gen_gmac_key(uint64_t index,
                                               const uint8_t *key_base,
                                               const uint8_t *iv,
                                               uint8_t *key_out) {
  uint8_t data[0x20];
  memcpy(data, key_base, 0x10);
  counter_add(data + 0x10, iv,
              index * CHIAKI_GKCRYPT_GMAC_KEY_REFRESH_IV_OFFSET);
  uint8_t md[0x20];
#ifdef CHIAKI_LIB_ENABLE_MBEDTLS
  // last param
...
CHIAKI_EXPORT void chiaki_gkcrypt_gen_new_gmac_key(ChiakiGKCrypt *gkcrypt,
                                                   uint64_t index) {
  assert(index > 0);
  chiaki_gkcrypt_gen_gmac_key(index, gkcrypt->key_gmac_base, gkcrypt->iv,
                              gkcrypt->key_gmac_current);
  gkcrypt->key_gmac_index_current = index;
}

CHIAKI_EXPORT void chiaki_gkcrypt_gen_tmp_gmac_key(ChiakiGKCrypt *gkcrypt,
                                                   uint64_t index,
                                                   uint8_t *key_out) {
  if (index)
    chiaki_gkcrypt_gen_gmac_key(index, gkcrypt->key_gmac_base, gkcrypt->iv,
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_gkcrypt_gen_key_stream(
    ChiakiGKCrypt *gkcrypt, uint64_t key_pos, uint8_t *buf, size_t buf_size) {
  ...
  if (!EVP_EncryptInit_ex(ctx, EVP_aes_128_ecb(), NULL, gkcrypt->key_base,
                          NULL)) {
  ...
  for (uint8_t *cur = buf, *end = buf + buf_size; cur < end;
       cur += CHIAKI_GKCRYPT_BLOCK_SIZE)
    counter_add(cur, gkcrypt->iv, counter_offset++);

#ifdef CHIAKI_LIB_ENABLE_MBEDTLS
  for (int i = 0; i < buf_size; i = i + 16) {
    // loop over all blocks of 16 bytes (128 bits)
    if (mbedtls_aes_crypt_ecb(&ctx, MBEDTLS_AES_ENCRYPT, buf + i, buf + i) !=
        0) {
      mbedtls_aes_free(&ctx);
      return CHIAKI_ERR_UNKNOWN;
    }
...
static bool gkcrypt_key_buf_should_generate(ChiakiGKCrypt *gkcrypt) {
  return gkcrypt->last_key_pos >
         gkcrypt->key_buf_key_pos_min + gkcrypt->key_buf_populated / 2;
}

CHIAKI_EXPORT ChiakiErrorCode chiaki_gkcrypt_get_key_stream(
    ChiakiGKCrypt *gkcrypt, uint64_t key_pos, uint8_t *buf, size_t buf_size) {
  if (!gkcrypt->key_buf)
  ...
  if (key_pos < gkcrypt->key_buf_key_pos_min ||
      key_pos + buf_size >=
          gkcrypt->key_buf_key_pos_min + gkcrypt->key_buf_populated) {
    CHIAKI_LOGW(gkcrypt->log,
                "Requested key stream for key pos %#llx on GKCrypt %d, but "
                "it's not in the buffer:"
                " key buf size %#llx, start offset: %#llx, populated: %#llx, "
                "min key pos: %#llx, last key pos: %#llx",
                (unsigned long long)key_pos, gkcrypt->index,
    ...
    size_t offset_in_buf =
        key_pos - gkcrypt->key_buf_key_pos_min + gkcrypt->key_buf_start_offset;
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_gkcrypt_decrypt(ChiakiGKCrypt *gkcrypt,
                                                     uint64_t key_pos,
                                                     uint8_t *buf,
                                                     size_t buf_size) {
  uint64_t padding_pre = key_pos % CHIAKI_GKCRYPT_BLOCK_SIZE;
  size_t full_size = ((padding_pre + buf_size + CHIAKI_GKCRYPT_BLOCK_SIZE - 1) /
                      CHIAKI_GKCRYPT_BLOCK_SIZE) *
                     CHIAKI_GKCRYPT_BLOCK_SIZE;
  ...
  ChiakiErrorCode err = chiaki_gkcrypt_get_key_stream(
      gkcrypt, key_pos - padding_pre, key_stream, full_size);
  if (err != CHIAKI_ERR_SUCCESS) {
    free(key_stream);
    return err;
  }
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_gkcrypt_gmac(ChiakiGKCrypt *gkcrypt,
                                                  uint64_t key_pos,
                                                  const uint8_t *buf,
                                                  size_t buf_size,
                                                  uint8_t *gmac_out) {
  ...
  if (mbedtls_gcm_setkey(&actx, MBEDTLS_CIPHER_ID_AES, gmac_key,
                         CHIAKI_GKCRYPT_BLOCK_SIZE * 8) != 0) {
    mbedtls_gcm_free(&actx);
    return CHIAKI_ERR_UNKNOWN;
  }

  // Use mbedtls_gcm_crypt_and_tag for GMAC (AAD only, no plaintext)
  // mbedtls v2.28 API: mbedtls_gcm_crypt_and_tag(ctx, mode, length, iv, iv_len,
  // add, add_len, input, output, tag_len, tag)
  uint8_t dummy_output[1]; // Not used for GMAC
  if (mbedtls_gcm_crypt_and_tag(&actx, MBEDTLS_GCM_ENCRYPT, 0, iv,
                                CHIAKI_GKCRYPT_BLOCK_SIZE, // no plaintext, iv
                                buf,
                                buf_size, // AAD (additional authenticated data)
                                NULL, dummy_output, // no input/output for GMAC
  ...
  if (!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN,
                           CHIAKI_GKCRYPT_BLOCK_SIZE, NULL)) {
    ret = CHIAKI_ERR_UNKNOWN;
    goto fail_cipher;
  }
  ...
  if (!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, CHIAKI_GKCRYPT_GMAC_SIZE,
                           gmac_out)) {
    ret = CHIAKI_ERR_UNKNOWN;
    goto fail_cipher;
  }
...
static ChiakiErrorCode gkcrypt_generate_next_chunk(ChiakiGKCrypt *gkcrypt) {
  assert(gkcrypt->key_buf_populated + KEY_BUF_CHUNK_SIZE <=
         gkcrypt->key_buf_size);
  size_t buf_offset =
      (gkcrypt->key_buf_start_offset + gkcrypt->key_buf_populated) %
      gkcrypt->key_buf_size;
  ...
  ChiakiErrorCode err = chiaki_gkcrypt_gen_key_stream(
      gkcrypt, key_pos, buf_start, KEY_BUF_CHUNK_SIZE);
  if (err != CHIAKI_ERR_SUCCESS)
...
static void *gkcrypt_thread_func(void *user) {
  ...
  while (1) {
    err = chiaki_cond_wait_pred(&gkcrypt->key_buf_cond, &gkcrypt->key_buf_mutex,
                                key_buf_mutex_pred, gkcrypt);

    if (gkcrypt->key_buf_thread_stop || err != CHIAKI_ERR_SUCCESS)
      break;

    /*
    CHIAKI_LOGV(gkcrypt->log, "GKCrypt %d key buf size %#llx, start offset:
    %#llx, populated: %#llx, min key pos: %#llx, last key pos: %#llx, generating
    next chunk", (int)gkcrypt->index, (unsigned long long)gkcrypt->key_buf_size,
                            (unsigned long long)gkcrypt->key_buf_start_offset,
                            (unsigned long long)gkcrypt->key_buf_populated,
                            (unsigned long long)gkcrypt->key_buf_key_pos_min,
                            (unsigned long long)gkcrypt->last_key_pos);
    */
    ...
    if (gkcrypt->last_key_pos >
        gkcrypt->key_buf_key_pos_min + gkcrypt->key_buf_populated) {
      // skip ahead if the last key pos is already beyond our buffer
      uint64_t key_pos =
          (gkcrypt->last_key_pos / KEY_BUF_CHUNK_SIZE) * KEY_BUF_CHUNK_SIZE;
      CHIAKI_LOGW(gkcrypt->log,
                  "Already requested a higher key pos than in the buffer, "
                  "skipping ahead from min %#llx to %#llx",
                  (unsigned long long)gkcrypt->key_buf_key_pos_min,
      ...
      gkcrypt->key_buf_start_offset =
          (gkcrypt->key_buf_start_offset + KEY_BUF_CHUNK_SIZE) %
          gkcrypt->key_buf_size;
...
CHIAKI_EXPORT void chiaki_key_state_commit(ChiakiKeyState *state,
                                           uint64_t prev) {
  state->prev = prev;
}
